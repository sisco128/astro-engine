...
const manifest = {
  ...
  profiles: {
    ...
    full: {
      description: `${Math.abs(Math.min(...years))} BCE - ${Math.max(...years) + 599} CE. Maximum freely available coverage. Default.`,
...
console.log(`  range:   ${Math.abs(Math.min(...years))} BCE -> ${Math.max(...years) + 599} CE`);
